...
module.exports = {
	...
	async execute(interaction) {
		...
		try {
			...
			const embedToMem = new MessageEmbed()
				.setColor(interaction.client.config.theme.red)
				.setAuthor(`You have been banned from ${interaction.guild.name}!`)
				.setDescription(`**• Member:** ${member.user.tag} (${member.user.id})\n**• Moderator:** ${interaction.member.user.tag}\n**• Reason:** ${reason}\n**• Case ID:** ${caseid}\n\n[Ban appeal](https://forms.gle/wPR1c977eXFpebVK7)`)
				.setThumbnail(member.user.avatarURL({ dynamic: true }));
			
			member.send({ embeds: [embedToMem] });
